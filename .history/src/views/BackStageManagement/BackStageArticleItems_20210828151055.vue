<template>
  <div id="articleItems">
    <div class="container">
      ...
      <div class="article">
        <h1 ref="content">{{article.title}}</h1>
        <p>
          {{article.content}}
        </p>
      </div>
...
export default {
  ...
  mounted () {
    this.$refs.content.insertAdjacentHTML('afterend', this.article.content)
  },
  methods: {
    audit (id) {
      console.log(id)
      this.$confirm('是否确定该博客通过审核?', '提示', {
        ...
        articleAudit({ blogId: id }).then(res => {
          console.log(res)
        }).catch(err => {
          console.log(err);
        })
    ...
    remove (id) {
      ...
      this.$confirm('是否确定删除该博客通?', '警告', {
        ...
        articleRemove({ blogId: id }).then(res => {
          console.log(res);
          location.reload();
        }).catch(err => {
          console.log(err);
