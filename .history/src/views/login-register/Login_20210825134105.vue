...
export default {
  ...
  data () {
    return {
      ...
      verificationCode: 'api/user/GetCode?time' + new Date(),
      loading: false,
      rules: {
        email: [
  ...
  methods: {
    getVerificationCodeURL () {
      this.getVerificationCodeURL = 'api/user/GetCode?time' + new Data()
    },
    onSubmit () {
      ...
      this.$refs['formLogin'].validate(valid => {
        if (!valid) return;
        this.loading = true;
        login(qs.stringify(this.user)).then(res => {
          console.log(res);
          this.$message({
            message: '登陆成功',
            type: 'success'
